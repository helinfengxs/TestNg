...
public class PermQueryCases {
    public HashMap<String,Object> loginData = new HashMap<>();
    @BeforeClass
    public void init(){
        loginData = Util.analyseData("PermQueryCases");
        RestAssured.baseURI = GlobalParams.request.get("base_url");
    }
    @Test(description = "查询权限")
    public void permQuery(){
        //传入所有模块数据对象，再根据yaml文件中的用例方法名获取到获取单个用例数据，
        CaseRequestParam loginCorrect = Util.getCaseData(loginData,"permQuery");
