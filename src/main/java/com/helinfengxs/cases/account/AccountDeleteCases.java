...
public class AccountDeleteCases {
    public HashMap<String,Object> loginData = new HashMap<>();
    @BeforeClass
    public void init(){
        loginData = Util.analyseData("AccountCases");
        RestAssured.baseURI = GlobalParams.request.get("base_url");
    }
    @Test(description = "删除账户")
    public void accountDelete(){

        //传入所有模块数据对象，再根据yaml文件中的用例方法名获取到获取单个用例数据，
        CaseRequestParam loginCorrect = Util.getCaseData(loginData,"Delete");
