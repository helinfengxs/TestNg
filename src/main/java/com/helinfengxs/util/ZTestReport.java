...
public class ZTestReport implements IReporter {
	...
	private final String path =System.getProperty("user.dir") +  File.separator+"report.html";

	private final String templatePath =System.getProperty("user.dir") + File.separator + "template";
	...
	private void outputResult(List<ITestResult> list) {
		...
		try {
			...
			BufferedWriter output = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(path)), StandardCharsets.UTF_8));
